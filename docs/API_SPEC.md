...
### Authentication
- [ ] POST /auth/signup (자체 인증)
- [ ] POST /auth/login (자체 인증)
- [ ] POST /auth/logout (토큰 무효화)
- [ ] POST /auth/forgot-password (이메일 발송)

### Records CRUD
- [ ] POST /api/records (insert + 세션 검증)
- [ ] GET /api/records (select + 페이징 + 필터)
- [ ] GET /api/records/{id} (select by id + 세션 검증)
- [ ] PATCH /api/records/{id} (update + 세션 검증)
- [ ] DELETE /api/records/{id} (soft delete + 세션 검증)

### Error Handling
- [ ] 입력 검증 (email, password, intensity 범위 등)
- [ ] 세션 기반 접근 제어 (다른 사용자 접근 불가)
- [ ] 에러 메시지 표준화
- [ ] 로깅 (에러, API 호출 시간 등)
